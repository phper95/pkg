...
func (p *Pool) PutWait(f Function) {
	if p.stopping {
		routineLogger.Print("routinepool[%v] was stopping, can not PutWait(task).", p.Name)
		return
	}
	p.checkRunningPanic()
...
func (p *Pool) put(task Task) bool {
	if p.stopping {
		routineLogger.Print("routinepool[%v] was stopping, can not put(task).", p.Name)
		return false
	}
	p.checkRunningPanic()
	select {
...
func (p *Pool) executeJob(task Task, timeout time.Duration) {
	...
	if p.currGorountine >= int64(p.numWorkers*4) {
		...
		p.reput(task)
		routineLogger.Print("routinepool[%s] numWorkers=[%v] but gorountine[%v] was running, re-put the job.",
	...
	go func() { // define function For recover() and cancel()
		...
		defer func() {
			...
			if e != nil {
				s := errors.Stack(2)
				log.Fatal("routinepool[%v] Panic: %v\nTraceback\r:%s",
					p.Name, e, string(s))
			}
		}()
		...
		if time.Since(start) > timeout {
			routineLogger.Print("Job runing timeout, limit[%v] used-time[%v] in routinepool[%v]",
				timeout, time.Since(start), p.Name)
		}

		//if routinepoolMetrics != nil {
		//	routinepoolMetrics.updateDelayHistogram(p.Name, task.GetTaskName(), start)
		//}
	}()
...
func (p *Pool) run(n int) {
	...
	defer routineLogger.Print("routinepool[%v] worker[%v].Done=[%v] exist loop. JobQueue.len=[%v]",
	...
	for {
		...
		case stop = <-worker.Stop:
			routineLogger.Print("routinepool[%v] worker[%v] stop=%v", p.Name, n, stop)
			stopTime = time.Now()
			if !stop {
				close(worker.Stop)
			}
...
func (p *Pool) Stop() {
	...
	if p.QueueLen() > 0 {
		routineLogger.Print("routinepool[%v] when Pool.Stop() had [%v]jobs not-finish.",
			p.Name, p.QueueLen())
	}
	...
	routineLogger.Print("Stop routine pool", p.Name, "currGorountine", p.currGorountine, "JobQueue len", p.QueueLen())
}
...
func (p *Pool) StopWait() {
	...
	for p.QueueLen() > 0 || p.currGorountine > 0 {
		...
		routineLogger.Print("==--StopWait()--==> routinepool[%v] currGorountine[%v].Done=[%v] JobQueue.len=[%v]",
